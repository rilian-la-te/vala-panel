...
#define xfce_tasklist_get_panel_plugin(tasklist) gtk_widget_get_ancestor (GTK_WIDGET (tasklist), vala_panel_applet_plugin_get_type())
#define xfce_tasklist_get_toplevel(tasklist) gtk_widget_get_ancestor (GTK_WIDGET (tasklist), vala_panel_toplevel_get_type())
...
static void _vala_panel_applet_menu_position_func_gtk_menu_position_func (GtkMenu* menu, gint* x, gint* y, gboolean* push_in, gpointer self) {
    vala_panel_applet_menu_position_func ((PanelApplet*) self, menu, x, y, push_in);
    g_object_unref (self);
}
  ...
  if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (button)))
    {
      ...
      gtk_menu_popup (GTK_MENU (menu), NULL, NULL,
                      _vala_panel_applet_menu_position_func_gtk_menu_position_func,
                              ...
                              gpointer      user_data)
{
  ...
  g_return_val_if_fail (a->type == CHILD_TYPE_GROUP
                            || WNCK_IS_WINDOW (a->window), 0);
  g_return_val_if_fail (b->type == CHILD_TYPE_GROUP
      ...
      if (class_group_a != class_group_b)
        {
          ...
          if (vala_panel_str_is_empty (name_a)
              && a->window != NULL)
            name_a = wnck_window_get_name (a->window);
          if (vala_panel_str_is_empty (name_b)
              && b->window != NULL)
                                   ...
                                   XfceTasklistChild *child)
{
  ...
  icon_size = vala_panel_toplevel_get_icon_size (VALA_PANEL_TOPLEVEL (xfce_tasklist_get_toplevel (tasklist)));
  ...
  if (event->button == 3)
    {
      ...
      gtk_menu_attach_to_widget (GTK_MENU (menu), button, NULL);
      gtk_menu_popup (GTK_MENU (menu), NULL, NULL,
                      child->type == CHILD_TYPE_WINDOW ? _vala_panel_applet_menu_position_func_gtk_menu_position_func : NULL,
                                         ...
                                         XfceTasklistChild *group_child)
{
  ...
  icon_size = vala_panel_toplevel_get_icon_size (VALA_PANEL_TOPLEVEL (xfce_tasklist_get_toplevel (group_child->tasklist)));
