...
#define xfce_taskbar_lock(tasklist) 0
#define xfce_taskbar_unlock(tasklist) 0
#define xfce_taskbar_is_locked(tasklist) 0
...
static void xfce_tasklist_get_property(GObject *object, guint prop_id, GValue *value,
                                       GParamSpec *pspec);
...
static void xfce_tasklist_forall(GtkContainer *container, gboolean include_internals,
...
static gint xfce_tasklist_button_compare(gconstpointer child_a, gconstpointer child_b,
                                         gpointer user_data);
...
static gint xfce_tasklist_size_sort_window(gconstpointer a, gconstpointer b)
{
...
static void xfce_tasklist_size_layout(XfceTasklist *tasklist, GtkAllocation *alloc, gint *n_rows,
                                      gint *n_cols, gint *arrow_position)
{
	gint rows;
	gint min_button_length;
	gint cols;
...
static void xfce_tasklist_size_allocate(GtkWidget *widget, GtkAllocation *allocation)
{
	...
	g_return_if_fail(gtk_widget_get_visible(tasklist->arrow_button));
...
static void xfce_tasklist_connect_screen(XfceTasklist *tasklist)
{
	...
	g_signal_connect(G_OBJECT(tasklist->gdk_display),
	                 "monitors-changed",
	                 G_CALLBACK(xfce_tasklist_gdk_screen_changed),
	                 tasklist);
	g_signal_connect(G_OBJECT(tasklist->gdk_display),
	                 "size-changed",
	                 G_CALLBACK(xfce_tasklist_gdk_screen_changed),
	...
	xfce_tasklist_gdk_screen_changed(tasklist->gdk_display, tasklist);
}
...
static void xfce_tasklist_disconnect_screen(XfceTasklist *tasklist)
{
	...
	g_return_if_fail(GDK_IS_SCREEN(tasklist->gdk_display));
	...
	g_signal_handlers_disconnect_by_func(G_OBJECT(tasklist->gdk_display),
	                                     G_CALLBACK(xfce_tasklist_gdk_screen_changed),
	                                     tasklist);
...
{
	g_return_if_fail(XFCE_IS_TASKLIST(tasklist));
	g_return_if_fail(GDK_IS_SCREEN(gdk_screen));
                                                   ...
                                                   XfceTasklist *tasklist)
{
	...
	if (xfce_taskbar_is_locked(tasklist) ||
	    (previous_workspace != NULL && tasklist->all_workspaces))
		return;
...
static gboolean xfce_tasklist_update_monitor_geometry_idle(gpointer data)
{
	XfceTasklist *tasklist = XFCE_TASKLIST(data);
	GdkScreen *screen;
	...
	if (!tasklist->all_monitors)
	{
		             ...
		             (tasklist->n_monitors = gdk_display_get_n_monitors(screen)) > 1))
		{
			...
			for (int tmp = 0; tmp < tasklist->n_monitors; tmp++)
			{
...
static gboolean xfce_tasklist_child_drag_motion_timeout(gpointer data)
{
...
static gboolean xfce_tasklist_child_drag_motion(XfceTasklistChild *child, GdkDragContext *context,
...
static void xfce_tasklist_child_drag_leave(XfceTasklistChild *child, GdkDragContext *context,
...
static gboolean xfce_tasklist_button_size_allocate(GtkWidget *widget, GdkRectangle *allocation,
                                                   gpointer user_data)
{
...
static gboolean xfce_tasklist_button_leave_notify_event(GtkWidget *button, GdkEventCrossing *event,
                                                        XfceTasklistChild *child)
{
	g_return_val_if_fail(XFCE_IS_TASKLIST(child->tasklist), false);
...
static gboolean xfce_tasklist_button_enter_notify_event(GtkWidget *button, GdkEventCrossing *event,
                                                        XfceTasklistChild *child)
{
	g_return_val_if_fail(XFCE_IS_TASKLIST(child->tasklist), false);
	g_return_val_if_fail(child->type != CHILD_TYPE_GROUP, false);
...
static gboolean xfce_tasklist_button_button_press_event(GtkWidget *button, GdkEventButton *event,
                                                        XfceTasklistChild *child)
{
	...
	if (event->type != GDK_BUTTON_PRESS || xfce_taskbar_is_locked(child->tasklist))
		return false;
...
static gboolean xfce_tasklist_button_button_release_event(GtkWidget *button, GdkEventButton *event,
                                                          XfceTasklistChild *child)
{
	...
	if (event->type == GDK_BUTTON_RELEASE && !xfce_taskbar_is_locked(child->tasklist) &&
...
static void xfce_tasklist_button_enter_notify_event_disconnected(gpointer data, GClosure *closure)
{
	XfceTasklistChild *child = data;
...
static void xfce_tasklist_button_drag_data_get(GtkWidget *button, GdkDragContext *context,
                                               GtkSelectionData *selection_data, guint info,
                                               guint timestamp, XfceTasklistChild *child)
{
	gulong xid;
...
static void xfce_tasklist_button_drag_begin(GtkWidget *button, GdkDragContext *context,
                                            XfceTasklistChild *child)
{
	GdkPixbuf *pixbuf;
	/* GdkPixmap *pixmap; */
...
static void xfce_tasklist_button_drag_data_received(GtkWidget *button, GdkDragContext *context,
                                                    gint x, gint y,
                                                    GtkSelectionData *selection_data, guint info,
                                                    guint drag_time, XfceTasklistChild *child2)
{
	GList *li, *sibling;
	gulong xid;
	XfceTasklistChild *child;
...
static gboolean xfce_tasklist_group_button_button_press_event(GtkWidget *button,
                                                              ...
                                                              XfceTasklistChild *group_child)
{
	...
	if (event->type != GDK_BUTTON_PRESS || xfce_taskbar_is_locked(group_child->tasklist))
		return false;
