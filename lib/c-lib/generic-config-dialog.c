...
static void set_file_response(GtkFileChooserButton *widget,
                              gpointer              user_data)
{
...
GtkDialog* generic_config_dlg(const char* title, GtkWindow* parent,
                                ValaPanelAppletWidget* applet, ...)
{
    va_list l;
    va_start(l,applet);
    GtkDialog* dlg = GTK_DIALOG(gtk_dialog_new_with_buttons( title, parent, GTK_DIALOG_DESTROY_WITH_PARENT,
                                         _("_Close"),
                                          GTK_RESPONSE_CLOSE,
                                          NULL ));
    ...
    while( TRUE )
    {
        ...
        else switch( type )
        {
            ...
            case CONF_DIRECTORY:
            {
                entry = gtk_file_chooser_button_new(_("Select a file"), CONF_FILE ? GTK_FILE_CHOOSER_ACTION_OPEN : GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER);
                g_autofree char* str = g_settings_get_string(settings,key);
                gtk_file_chooser_set_filename(GTK_FILE_CHOOSER(entry),str);
                g_autofree SignalData* data = (SignalData*)g_malloc(sizeof(SignalData));
            ...
            case CONF_TRIM:
            {
                ...
                g_autofree char* markup = g_markup_printf_escaped ("<span style=\"italic\">%s</span>", name);
